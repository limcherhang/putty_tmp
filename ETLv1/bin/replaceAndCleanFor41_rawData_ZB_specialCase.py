...
def process_delete(schema: str, table: str, start: datetime.datetime, end: datetime.datetime, cursor: pymysql.cursors.Cursor):
    while True:
        if "API" in table and table != 'RESTAPI':
            ...
            delete_sql = f"""
                DELETE FROM {schema}.{table} WHERE DBts >= '{start}' AND DBts < '{end}' LIMIT 10000;
            """
        else:
            delete_sql = f"""
                ...
                while date.timestamp() < next_date.timestamp():
                    ...
                    if "API" in module and module != 'RESTAPI':
                        ...
                        if errorCode == 1:
                            ...
                            if module == 'ZB':
                                sqlCommand = f"""
                                    SELECT DBts, GWts, ZBts, ieee, clusterID FROM {process_schema}.{module} WHERE DBts >= '{date}' AND DBts < '{next_time}' GROUP BY DBts, GWts, ZBts, ieee, clusterID
                                    EXCEPT
                                    SELECT DBts, GWts, ZBts, ieee, clusterID FROM {process_schema}{write_year}.{module}_{write_month} WHERE DBts >= '{date}' AND DBts < '{next_time}'
                                """
                            else:
                                sqlCommand = f"""
