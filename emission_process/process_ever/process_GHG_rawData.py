...
if __name__ == '__main__':
    ...
    with conn.cursor() as cursor:
        ...
        for ID, Type, rawData, Year, updateTime in cursor.fetchall():
            ...
            if "Gas basis kg/m3" in rawData:
                rawData['Gas basiskg/m3'] = rawData['Gas basis kg/m3']
            if "Energy basis kg/TJ" in rawData:
                rawData['Energy basiskg/TJ'] = rawData['Energy basis kg/TJ']
            if "Mass basis kg/tonne" in rawData:
                rawData['Mass basiskg/tonne'] = rawData['Mass basis kg/tonne']
            if "Liquid basis kg/ litre" in rawData:
                rawData['Liquid basiskg/ litre'] = rawData['Liquid basis kg/ litre']
            if "Lower heating Value  TJ/Gg" in rawData:
                rawData['Lower heating Value TJ/Gg'] = rawData['Lower heating Value  TJ/Gg']
            if "Fuel density information1 Of gases (kg/m3 of fuel)" in rawData:
                rawData['Fuel density information1Of gases (kg/m3 of fuel)'] = rawData['Fuel density information1 Of gases (kg/m3 of fuel)']
            if "Fuel density information1 Of liquids (kg/litre fuel)" in rawData:
                rawData['Fuel density information1Of liquids (kg/litre fuel)'] = rawData['Fuel density information1 Of liquids (kg/litre fuel)']
            ...
            if 'CO2' in Type:
                EF_content[fuel]["CO2"] = {
                    "FuelType": rawData['Fuel Type'] if rawData['Fuel Type'] != 'Null' else None,
        ...
        for sourceOfEmission, ghgInfo in EF_content.items():
            ...
            if co2m3 is not None or ch4m3 is not None or n2om3 is not None:
                ...
                value_string = f"('{EFId}', '{sourceOfEmission}', '{_type}', '{co2m3 if co2m3 is not None else 'NULL'}', '{ch4m3 if ch4m3 is not None else 'NULL'}', '{n2om3 if n2om3 is not None else 'NULL'}', 'm3', 2017, 'GHG EF from Cross-Sector Tools', 'Sheet Name : Stationary combustion', '{'kgCO₂' if co2m3 is not None else 'NULL'}', '{'kgCH₄' if ch4m3 is not None else 'NULL'}', '{'kgN₂O' if n2om3 is not None else 'NULL'}', {sourceId}, '{ever_table}', '{', '.join(str(x) for x in everIds)}')".replace("'NULL'", "NULL")
            ...
            if co2TJ is not None or ch4TJ is not None or n2oTJ is not None:
                ...
                value_string = f"('{EFId}', '{sourceOfEmission}', '{_type}', '{co2TJ if co2TJ is not None else 'NULL'}', '{ch4TJ if ch4TJ is not None else 'NULL'}', '{n2oTJ if n2oTJ is not None else 'NULL'}', 'TJ', 2017, 'GHG EF from Cross-Sector Tools', 'Sheet Name : Stationary combustion', '{'kgCO₂' if co2TJ is not None else 'NULL'}', '{'kgCH₄' if ch4TJ is not None else 'NULL'}', '{'kgN₂O' if n2oTJ is not None else 'NULL'}', {sourceId}, '{ever_table}', '{', '.join(str(x) for x in everIds)}')".replace("'NULL'", "NULL")
            ...
            if co2tonne is not None or ch4tonne is not None or n2otonne is not None:
                ...
                value_string = f"('{EFId}', '{sourceOfEmission}', '{_type}', '{co2tonne if co2tonne is not None else 'NULL'}', '{ch4tonne if ch4tonne is not None else 'NULL'}', '{n2otonne if n2otonne is not None else 'NULL'}', 'tonne', 2017, 'GHG EF from Cross-Sector Tools', 'Sheet Name : Stationary combustion', '{'kgCO₂' if co2tonne is not None else 'NULL'}', '{'kgCH₄' if ch4tonne is not None else 'NULL'}', '{'kgN₂O' if n2otonne is not None else 'NULL'}', {sourceId}, '{ever_table}', '{', '.join(str(x) for x in everIds)}')".replace("'NULL'", "NULL")
            ...
            if co2liter is not None or ch4liter is not None or n2oliter is not None:
                ...
                value_string = f"('{EFId}', '{sourceOfEmission}', '{_type}', '{co2liter if co2liter is not None else 'NULL'}', '{ch4liter if ch4liter is not None else 'NULL'}', '{n2oliter if n2oliter is not None else 'NULL'}', 'Liter', 2017, 'GHG EF from Cross-Sector Tools', 'Sheet Name : Stationary combustion', '{'kgCO₂' if co2liter is not None else 'NULL'}', '{'kgCH₄' if ch4liter is not None else 'NULL'}', '{'kgN₂O' if n2oliter is not None else 'NULL'}', {sourceId}, '{ever_table}', '{', '.join(str(x) for x in everIds)}')".replace("'NULL'", "NULL")
            ...
            if co2m32 is not None or ch4m32 is not None or n2om32 is not None:
                ...
                value_string = f"('{EFId}', '{sourceOfEmission}', '{_type}', '{co2m32 if co2m32 is not None else 'NULL'}', '{ch4m32 if ch4m32 is not None else 'NULL'}', '{n2om32 if n2om32 is not None else 'NULL'}', 'm3', 2017, 'GHG EF from Cross-Sector Tools', 'Sheet Name : Stationary combustion', '{'kgCO₂' if co2m32 is not None else 'NULL'}', '{'kgCH₄' if ch4m32 is not None else 'NULL'}', '{'kgN₂O' if n2om32 is not None else 'NULL'}', {sourceId}, '{ever_table}', '{', '.join(str(x) for x in everIds)}')".replace("'NULL'", "NULL")
