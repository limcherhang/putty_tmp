...
def main(conn,start):
    ...
    with conn.cursor() as cursor:
        ...
        for data in NameList:
            ...
            sqlCommand=f"SELECT IFNULL(totalFirst, 0) FROM reportPlatform{year}.Dpower where month(date) = {month} and siteId = {sId} and name = '{name}' order by date asc limit 1; "
            try:
                logger.debug(sqlCommand)
                cursor.execute(sqlCommand)
            ...
            totalFirst = first[0]
            sqlCommand=f"SELECT IFNULL(total,0) FROM reportPlatform{year}.Dpower where month(date) = {month} and siteId = {sId} and name = '{name}' order by date desc limit 1 ; "
            try:
                logger.debug(sqlCommand)
                cursor.execute(sqlCommand)
            ...
            energyConsumption = total - totalFirst 
            value_string += f"({month},'{date}',{sId},'{name}',{energyConsumption},{totalFirst},{total}), " 

    if value_string != '':
        with conn.cursor() as cursor:
            sql = f"REPLACE INTO reportPlatform{year}.Mpower(month,updateDate,siteId,name,energyConsumption,totalFirst,total) VALUES {value_string[:-2]}"
            cursor.execute(sql)
            logger.debug(sql)
