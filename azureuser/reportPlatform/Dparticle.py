...
def main(conn,start,end):
    ...
    with conn.cursor() as cursor:
        sqlCommand = f"select siteId, name from mgmtETL.DataPlatform where name like 'particle#%' and gatewayId>0"
        try:
            ...
            NameList = ()
        logger.info(f"--------------- particle --------------- Took: {round(time.time()-s, 3)}s")
        for rows in NameList:
            sId = rows[0]
            name = rows[1]
            data_list = []
            logger.info(f"--------------- select sId:{sId} name:{name}--------------- Took: {round(time.time()-s, 3)}s")
            sqlCommand = f"select particle from dataPlatform.particle where siteId={sId} and name='{name}' and ts>='{start}' and ts<'{end}' order by ts asc"
            try:
                logger.debug(sqlCommand)
                cursor.execute(sqlCommand)
            ...
            if len(result) == 0:
                ...
                if len(data_list) != 0:
                    ...
                    value_string += f"('{date}', {sId}, '{name}', {particleMin}, {particle25th}, {particleMedian}, {particle75th}, {particleMax}), "
            
    if value_string != '':
        value_string = value_string[:-2]
        with conn.cursor() as cursor:
            replace_sql = f"replace into `reportPlatform{year}`.`Dparticle` (`date`, `siteId`, `name`, `particleMin`, `particle25th`, `particleMedian`, `particle75th`, `particleMax`) Values {value_string}"
