...
def main(conn,start,end):
    ...
    with conn.cursor() as cursor:
        sqlCommand = f"select siteId, name from mgmtETL.DataPlatform where name like 'temp#%' and gatewayId>0"
        try:
            ...
            NameList = ()
        logger.info(f"--------------- temp --------------- Took: {round(time.time()-s, 3)}s")
        for rows in NameList:
            sId = rows[0]
            name = rows[1]
            data_list = []
            logger.info(f"--------------- select sId:{sId} name:{name}--------------- Took: {round(time.time()-s, 3)}s")
            sqlCommand = f"select temp from dataPlatform.temp where ts>='{start}' and ts<'{end}' and siteId={sId} and name='{name}' order by ts asc"
            try:
                logger.debug(sqlCommand)
                cursor.execute(sqlCommand)
            ...
            if len(result) == 0:
                ...
                if len(data_list) != 0:
                    ...
                    value_string += f"('{date}', {sId}, '{name}', {tempMin}, {temp25th}, {tempMedian}, {temp75th}, {tempMax}), "
            
    if value_string != '':
        value_string = value_string[:-2]
        with conn.cursor() as cursor:
            replace_sql = f"replace into `reportPlatform{year}`.`Dtemp` (`date`, `siteId`, `name`, `tempMin`, `temp25th`, `tempMedian`, `temp75th`, `tempMax`) Values {value_string}"
