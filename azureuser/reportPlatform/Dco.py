...
def main(conn,start,end):
    ...
    with conn.cursor() as cursor:
        sqlCommand = f"select siteId, name from mgmtETL.DataPlatform where name like 'co#%' and gatewayId>0"
        try:
            ...
            NameList = ()
        logger.info(f"--------------- co --------------- Took: {round(time.time()-s, 3)}s")
        for rows in NameList:
            sId = rows[0]
            name = rows[1]
            data_list = []
            logger.info(f"--------------- select sId:{sId} name:{name}--------------- Took: {round(time.time()-s, 3)}s")
            sqlCommand = f"select CO from dataPlatform.co where siteId={sId} and name='{name}' and ts>='{start}' and ts<'{end}' order by ts asc"
            try:
                logger.debug(sqlCommand)
                cursor.execute(sqlCommand)
    ...
    if value_string != '':
        ...
        with conn.cursor() as cursor:
            replace_sql = f"replace into `reportPlatform{year}`.`Dco` (`date`, `siteId`, `name`, `COMin`, `CO25th`, `COMedian`, `CO75th`, `COMax`) Values {value_string}"
