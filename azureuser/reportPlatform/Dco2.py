...
def main(conn,start,end):
    ...
    with conn.cursor() as cursor:
        sqlCommand = f"select siteId, name from mgmtETL.DataPlatform where name like 'co2#%' and gatewayId>0"
        try:
            ...
            NameList = ()
        logger.info(f"--------------- co2 --------------- Took: {round(time.time()-s, 3)}s")
        for rows in NameList:
            sId = rows[0]
            name = rows[1]
            data_list = []
            logger.info(f"--------------- select sId:{sId} name:{name}--------------- Took: {round(time.time()-s, 3)}s")
            sqlCommand = f"select co2 from dataPlatform.co2 where siteId={sId} and name='{name}' and ts>='{start}' and ts<'{end}' order by ts asc"
            try:
                logger.debug(sqlCommand)
                cursor.execute(sqlCommand)
            ...
            if len(result) == 0:
                ...
                if len(data_list) != 0:
                    ...
                    value_string += f"('{date}', {sId}, '{name}', {co2Min}, {co225th}, {co2Median}, {co275th}, {co2Max}), "


    if value_string != '':
        value_string = value_string[:-2]
        with conn.cursor() as cursor:
            replace_sql = f"replace into `reportPlatform{year}`.`Dco2` (`date`, `siteId`, `name`, `co2Min`, `co225th`, `co2Median`, `co275th`, `co2Max`) Values {value_string}"
