...
def main(conn,st,et):
    ...
    with conn.cursor() as cursor:
        sql = f"SELECT gatewayId,deviceId,name FROM mgmtETL.DataETL  where deviceType = 2 and deviceLogic = 4" 
        try:
            logger.debug(sql)
            cursor.execute(sql)
            ...
            infos = []
        logger.info(f"--------------- select Type:2 Logic:4 Mapping --------------- Took: {round(time.time()-s, 3)}s")
        for row in infos:
            gId = row[0]
            dId = row[1]
            name = row[2]
            ...
            #sqlCommand = f"select GWts,rawData from rawData.bacnet where gatewayId = {gId} and name = '{dId_[1]}' order by GWts desc limit 1"
            sqlCommand = f"select GWts,rawData from rawData.bacnet where GWts >= '{st}' and GWts < '{et}' and gatewayId = {gId} and name = '{dId_[1]}'"
            try:
