...
def main(conn,st,et):
    ...
    with conn.cursor() as cursor:
        sql = f"SELECT gatewayId,deviceId,name FROM mgmtETL.DataETL  where deviceType = 1 and deviceLogic = 9" #要改
        try:
            logger.debug(sql)
            cursor.execute(sql)
            ...
            infos = []
        logger.info(f"--------------- select Type:1 Logic:9 Mapping --------------- Took: {round(time.time()-s, 3)}s")
        for rows in infos:
            gId = rows[0]
            dId = rows[1]
            name = rows[2]
