...
def main(conn,st,et):
    ...
    with conn.cursor() as cursor:
        sql = f"SELECT gatewayId,deviceId,name FROM mgmtETL.DataETL where deviceType = 12 and deviceLogic = 1" 
        try:
            logger.debug(sql)
            cursor.execute(sql)
            ...
            infos = []
        logger.info(f"--------------- select Type:12 Logic:1 Mapping --------------- Took: {round(time.time()-s, 3)}s")
        for row in infos:
            gId = row[0]
            dId = row[1]
            name = row[2]
