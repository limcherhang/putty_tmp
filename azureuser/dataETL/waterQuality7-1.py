...
def main(conn,st,et):
    ...
    with conn.cursor() as cursor:
        sql = f"SELECT gatewayId,deviceId,name FROM mgmtETL.DataETL where deviceType = 7 and deviceLogic = 1 " 
        try:
            logger.debug(sql)
            cursor.execute(sql)
            ...
            infos = []
        logger.info(f"--------------- select Type:7 Logic:1 Mapping --------------- Took: {round(time.time()-s, 3)}s")
        for row in infos:
            gId = row[0]
            dId = row[1]
            name = row[2]
            logger.info(f"--------------- select GatewayId:{gId} DeviceId:{dId}--------------- Took: {round(time.time()-s, 3)}s")
            sqlCommand = f"select ZBts,rawData from rawData.zigbee where gatewayId = {gId} and ieee = '{dId}' and GWts >= '{st}' and GWts < '{et}'"
            try:
            ...
            if len(datas) == 0:
                ...
                for data in datas:
                    ...
                    rawdata = data[1]
                    if len(rawdata) == 12:
                        ph = int(rawdata[:4],16) /100
                        temperature = int(rawdata[4:8],16) /10
                        oxidationReductionPotential = int(rawdata[8:12],16)
                        totalDissovedSolids = 'null'
                        electricConductivity = 'null'
                        electricResistivity = 'null'
                    elif len(rawdata) == 28:
                        ph = 'null'
                        oxidationReductionPotential = 'null'
                        electricConductivity = int(rawdata[:8],16) /100
                        temperature = int(rawdata[8:12],16) /10
                        totalDissovedSolids = int(rawdata[12:20],16) /100
                        electricResistivity = int(rawdata[20:28],16) /100
