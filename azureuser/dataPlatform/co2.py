...
def main(conn,st,et):
    ...
    with conn.cursor() as cursor:
        sqlCommand = f"select siteId,gatewayId,name,dataETLName from mgmtETL.DataPlatform where name like 'co2#%' and logicModel = 'normal' and logicModel is NOT NULL"
        try:
            ...
            infos = []
        logger.info(f"--------------- select co2 Mapping --------------- Took: {round(time.time()-s, 3)}s")    
        for rows in infos:
            sId = rows[0]
            gId = rows[1]
            name = rows[2]
            ...
            sqlCommand = f"select ts, co2 from dataETL.{table} where gatewayId={gId} and name='{ETLName}' and ts>='{st}' and ts<'{et}'" 
            try:
                logger.debug(sqlCommand)
                cursor.execute(sqlCommand)
