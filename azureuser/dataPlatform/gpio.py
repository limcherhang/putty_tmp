...
def main(conn,st,et):
    ...
    with conn.cursor() as cursor:
        sqlCommand="select siteId,gatewayId,name,dataETLName,value from mgmtETL.DataPlatform where name like 'gpio#%' and logicModel = 'value' and logicModel is NOT NULL"
        try:
            ...
            infos = []
        logger.info(f"--------------- select gpio Mapping --------------- Took: {round(time.time()-s, 3)}s")
        for rows in infos:
            sId = rows[0]
            gId = rows[1]
            name = rows[2]
            ...
            sqlCommand=f"Select * From dataETL.gpio Where gatewayId={gId} and name='{ETLName}' and ts>='{st}' and ts<'{et}'"
            try:
                logger.debug(sqlCommand)
                cursor.execute(sqlCommand)
            ...
            if len(datas) == 0:
                ...
                for data in datas:
                    ts = data[0]
                    statu = data[3+value]
                    value_string += f"('{ts}',{sId},'{name}',{statu}), "

    if value_string != '':
        value_string = value_string[:-2]
        with conn.cursor() as cursor:
            replace_sql=f"Replace into `dataPlatform`.`gpio`(`ts`,`siteId`,`name`,`status`)values{value_string}"
            try:
