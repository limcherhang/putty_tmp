...
def main(conn,st,et):
    ...
    with conn.cursor() as cursor:
        sqlCommand = f"select siteId,gatewayId,name,dataETLName from mgmtETL.DataPlatform where name like 'voc#%' and logicModel = 'normal'"
        try:
        ...
        for rows in infos:
            ...
            sqlCommand = f"select ts, VOC from dataETL.airQuality where gatewayId={gId} and name='{etlName}' and ts>='{st}' and ts<'{et}'"
            try:
                logger.debug(sqlCommand)
                cursor.execute(sqlCommand)
            ...
            if len(datas) == 0:
                ...
                for data in datas:
                    ...
                    ts = data[0]
                    voc = data[1]
                    value_string += f"('{ts}', {sId}, '{name}', {voc}), "

    if value_string != '':
        value_string = value_string[:-2]
        with conn.cursor() as cursor:
            replace_sql = f"replace into `dataPlatform`.`voc` (`ts`, `siteId`, `name`, `VOC`) Values {value_string}"
            try:
