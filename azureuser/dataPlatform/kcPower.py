...
def main(conn,st,et):
    value_string = ''   
    name_list = ['power#2','power#6','power#7','power#8','power#9','power#14']

    with conn.cursor() as cursor:
        sqlCommand = f"SELECT siteId,gatewayId,name,dataETLName FROM mgmtETL.DataPlatform where siteId = 87 and name like 'power#%' and logicModel = 'normal' ;"
        try:
            ...
            infos = []
        logger.info(f"--------------- select KC Power Mapping --------------- Took: {round(time.time()-s, 3)}s")
        for rows in infos:
            sId = rows[0]
            gId = rows[1]
            name = rows[2]
            ...
            if len(datas) == 0:
                ...
                for data in datas:
                    ...
                    if name == 'power#1' and sId == 87 and ch1watt < 0:
                        ch1watt = 0
                    elif name == 'power#14' and sId == 87 and ch1watt > 0:
                        ch1watt = 0
                        
                    power = ch1watt/1000

                    if name in name_list:
                        value_string += f"('{ts}',{sId},'{name}',{abs(ch1watt)},{abs(power)}, {energy}, {total}), "
                    else:
