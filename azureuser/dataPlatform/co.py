...
def main(conn,st,et):
    ...
    with conn.cursor() as cursor:
        sqlCommand = f"select siteId,gatewayId,name,dataETLName from mgmtETL.DataPlatform where name like 'co#%' and logicModel = 'normal' and logicModel is NOT NULL"
        try:
            ...
            infos = []
        logger.info(f"--------------- select co Mapping --------------- Took: {round(time.time()-s, 3)}s")
        for rows in infos:
            sId = rows[0]
            gId = rows[1]
            name = rows[2]
