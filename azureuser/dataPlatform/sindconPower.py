...
def getTotal(conn,name,st):
    ...
    with conn.cursor() as cursor:
        sqlCommand=f"select total from dataPlatform.power where ts >= '{stime}' and ts < '{etime}' and name = '{name}'  and siteId = 82 order by ts asc limit 1"
...
def main(conn,st,et):
    value_string = ''   
    with conn.cursor() as cursor:
        sqlCommand = f"SELECT siteId,gatewayId,name,dataETLName FROM mgmtETL.DataPlatform where siteId = 82 and name like 'power#%' and logicModel = 'normal' ;"
        try:
            ...
            infos = []
        logger.info(f"--------------- select Sindcon Power Mapping --------------- Took: {round(time.time()-s, 3)}s")
        for rows in infos:
            sId = rows[0]
            gId = rows[1]
            name = rows[2]
