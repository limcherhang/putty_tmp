...
def main(conn,st,et):
    ...
    with conn.cursor() as cursor:
        sqlCommand = f"select siteId,gatewayId,name,dataETLName from mgmtETL.DataPlatform where name like 'ch2o#%' and logicModel = 'normal'  and logicModel is NOT NULL"
        try:
            ...
            infos = []
        logger.info(f"--------------- select ch2o Mapping --------------- Took: {round(time.time()-s, 3)}s")
        for rows in infos:
            sId = rows[0]
            gId = rows[1]
            name = rows[2]
            ...
            sqlCommand = f"select ts,CH2O from dataETL.airQuality where gatewayId={gId} and name='{ETLName}' and ts>='{st}' and ts<'{et}'"
            try:
                logger.debug(sqlCommand)
                cursor.execute(sqlCommand)
            ...
            if len(datas) == 0:
                ...
                for data in datas:
                    ts = data[0]
                    ch2o = data[1]
                    value_string += f"('{ts}', {sId}, '{name}', {ch2o}), "
                
    if value_string != '':
        value_string = value_string[:-2]
        with conn.cursor() as cursor:
            replace_sql = f"replace into `dataPlatform`.`ch2o` (`ts`, `siteId`, `name`, `CH2O`) Values {value_string}"
            try:
