...
def main(conn,st,et):
    ...
    with conn.cursor() as cursor:
        sqlCommand = f"select siteId,gatewayId,name,dataETLName from mgmtETL.DataPlatform where name like 'temp#%' and logicModel = 'normal' and logicModel is NOT NULL"
        try:
            ...
            infos = []
        logger.info(f"--------------- select temp-normal Mapping --------------- Took: {round(time.time()-s, 3)}s")
        for rows in infos:
            sId = rows[0]
            gId = rows[1]
            name = rows[2]
            etlName = rows[3]
            string = etlName.split('#')

            if string[0] == 'TwoInOne':
                table = 'twoInOne'
            elif string[0] == 'ThreeInOne':
                table = 'threeInOne'
            elif string[0] == 'AirQuality':
                table = 'airQuality'
            else:
                table = 'temp'
            #table = string.lower()[0] + string[1:]
