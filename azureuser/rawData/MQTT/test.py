...
if __name__ == "__main__":
    ...
    logger = myLog.get_logger(logPath, f"{filename}.log", config["mysql_own"])
    ...
    while True:
        while dataQueue.qsize() != 0:
            ...
            if 'zb' in topic:
                logger.error(f"data: {data}, data type: {type(data)}, msg_list: {msg_list}, length of data received: {len(msg_list)}")
                logger.critical(f"zb: {len(msg_list)}")
            elif 'zigbee' in topic and len(msg_list) != 7:
                logger.debug(f"data: {data}, data type: {type(data)}, length of data received: {len(msg_list)}")
                logger.warning(f"zigbee: {len(msg_list)}")